class ActiveRecord::Base#.class_eval do
  def self.polymorphic_attr( base_attr, options = {} )
    ...
    attr_classes = Hash[*attr_types.map { |attr_as| [ attr_as, ( Object.const_get( ( attr_class_options[attr_as] || attr_as.to_s.classify))) ] }.flatten]
    
    attr_types.each do |attr_as|
      define_method("#{attr_as}?") { self.send(base_attr) and self.send(base_attr).is_a?(attr_classes[attr_as]) }
