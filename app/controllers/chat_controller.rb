...
class ChatController < ApplicationController
  ...
  def say
    ...
    if message.match(/^\/\w+/)
      ...
      when 'me'
        ...
        @message = Chat::Message.find(:first, :order => "id DESC", :conditions => ["sender_id = ?", @user.id])
      else
        return false
      end
    else
  ...
  def get_channel_and_user
    ...
    unless @channel
      ...
      if @group
        ...
        unless @channel
          @channel = Chat::Channel.create(:name => @group.name, :group_id => @group.id)
        end
      end
    end
  ...
  def user_say_in_channel(user, channel, say)
    ...
    Chat::Message.new(:channel => channel, :content => say, :sender => user).save
  end
  
  def user_action_in_channel(user, channel, say)
    Chat::Message.new(:channel => channel, :content => sanitize(say), :sender => user, :level => 'action').save
  end
  
  def user_joins_channel(user, channel)
    Chat::Message.new(:channel => channel, :sender => user, :content => "joins the chatroom", :level => 'sys').save
  end
  
  def user_leaves_channel(user, channel)
    Chat::Message.new(:channel => channel, :sender => user, :content => "left the chatroom", :level => 'sys').save
  end
