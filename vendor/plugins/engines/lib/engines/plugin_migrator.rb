...
class Engines::PluginMigrator < ActiveRecord::Migrator
  ...
  def self.current_version(plugin=current_plugin)
    return ActiveRecord::Base.connection.select_one("SELECT MAX(id) as max FROM #{schema_info_table_name}")["max"].to_i rescue 0
  ...
  def set_schema_version(version)
    if down?
      ActiveRecord::Base.connection.execute("DELETE FROM #{self.class.schema_info_table_name} WHERE id = #{version} AND plugin_name = '#{self.current_plugin.name}'")
    else
      ActiveRecord::Base.connection.execute("INSERT INTO #{self.class.schema_info_table_name} VALUES(#{version}, #{ActiveRecord::Base.connection.quote(Time.now)}, '#{self.current_plugin.name}')")
    end
    return
