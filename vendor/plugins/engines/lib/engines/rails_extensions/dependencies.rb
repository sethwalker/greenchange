    ...
    ['controller', 'helper'].each do |file_type| 
      ...
      if file_name =~ /^(.*app\/#{file_type}s\/)?(.*_#{file_type})(\.rb)?$/
        ...
        Rails.plugins.each do |plugin|
          ...
          logger.debug("checking plugin '#{plugin.name}' for '#{base_name}'") if $DEBUG
          if File.file?("#{plugin_file_name}.rb")
            logger.debug("==> loading from plugin '#{plugin.name}'") if $DEBUG
        ...
        if Engines.disable_application_code_loading
          logger.debug("loading from application disabled.") if $DEBUG
        else
          ...
          if File.file?("#{app_file_name}.rb")
            logger.debug("loading from application: #{base_name}") if $DEBUG
            file_loaded = true if require_or_load_without_engine_additions(app_file_name, const_path)
          else
            logger.debug("(file not found in application)") if $DEBUG
          end
        end        
      end 
    end
